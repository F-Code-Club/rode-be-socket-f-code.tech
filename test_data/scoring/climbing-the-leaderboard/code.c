...
int* remove_duplicate(int *reverse_sorted_array, int length, int *new_length) {
    int* result = malloc(length * sizeof(int));
    int j = 0;
    
    for(int i = 0; i < length; i++) {
        if (j == 0 || result[j - 1] != reverse_sorted_array[i]) {
            result[j] = reverse_sorted_array[i];
            j++;
        }
    }
    *new_length = j;
...
int reverse_binary_search(int value, int *reverse_sorted_array, int length) {
    int left = 0;
    int right = length;
    while (left < right) {
        int mid = (left + right) / 2;
        int element = reverse_sorted_array[mid];
        if (value == element) {
            return mid;
        }
        if(value > element) {
...
int* climbingLeaderboard(int ranked_count, int* ranked, int player_count, int* player, int* result_count) {
    *result_count = player_count;
    int new_ranked_length = 0;
    int* new_ranked = remove_duplicate(ranked, ranked_count, &new_ranked_length);
    int* result = malloc(*result_count * sizeof(int));
    for(int i = 0; i < player_count; i++) {
        result[i] = reverse_binary_search(player[i], new_ranked, new_ranked_length) + 1;
    }
    return result;
}
